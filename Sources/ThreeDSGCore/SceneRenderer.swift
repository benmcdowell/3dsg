...
public struct DeviceRenderer: Sendable {
    ...
    private func addLightingEnvironment(to scene: SCNScene) {
        scene.lightingEnvironment.contents = generatedStudioLightboxEnvironment()
        scene.lightingEnvironment.intensity = 2.6
    }

    private func generatedStudioLightboxEnvironment() -> NSImage {
        let width = 1024
        let height = 512
        ...
        let backgroundStops: [CGFloat] = [0, 0.32, 0.58, 1]
        if let gradient = CGGradient(colorsSpace: colorSpace, colors: backgroundColors, locations: backgroundStops) {
        ...
        drawLightboxPanels(in: context, size: size)
    ...
    private func drawLightboxPanels(in context: CGContext, size: CGSize) {
        ...
        for (index, centerRatio) in centers.enumerated() {
            let centerX = size.width * centerRatio
            let rect = CGRect(
            ...
            let brightness: CGFloat = index == 3 ? 1.0 : 0.88
